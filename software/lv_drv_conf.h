...
typedef uint8_t lv_i2c_handle_t;
...
static inline int lv_i2c_write(lv_i2c_handle_t i2c_dev, const uint8_t* reg, const void* data_out, uint16_t datalen)
{
	...
	i2c_master_write_byte(cmd, (i2c_dev << 1) | I2C_MASTER_WRITE, 1);
    i2c_master_write_byte(cmd, *reg, 1);
    for(register int lenght = 0; lenght < datalen; lenght++)
    {
 ...
 * @param i2c_dev Pointer to i2c device
 * @param reg Pointer to register address to read if non-null
 * @param data_out Pointer to data buffer to read if non-null
...
static inline int lv_i2c_read(lv_i2c_handle_t i2c_dev, const uint8_t* reg, void* data_in, uint16_t datalen)
{
    ...
    // for(register int lenght = 0; lenght < datalen; lenght++)
    // {
    i2c_master_read_byte(cmd, data_in, 1);
        // data_in = &data;
    // }
